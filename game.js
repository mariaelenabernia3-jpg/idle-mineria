document.addEventListener('DOMContentLoaded', () => {
    ...
    const firebaseConfig = {
      apiKey: "AIzaSyDZXqVF1FjNIXriEcCKzO5jpDW1lNJC6yI", // Revisa que tu API Key sea correcta
    ...
    function init() {
        ...
        DOM.logoutBtn.onclick = gameLogic.signOut;
    ...
    Object.assign(gameLogic, {
        handleAuthStateChanges: () => { auth.onAuthStateChanged(user => user ? gameLogic.load() : stopGame()); },
        signInWithGoogle: () => { auth.signInWithPopup(new firebase.auth.GoogleAuthProvider()).catch(e => console.error("Error de login:", e)); },
        signOut: () => { gameLogic.save().then(() => auth.signOut()); },
        getDefaultState: () => ({
        ...
        save: () => {
            ...
            return db.collection('players').doc(auth.currentUser.uid).set(state, { merge: true })
                .then(gameLogic.updateLeaderboard).catch(e => console.error("Error de guardado:", e));
        },
        load: () => {
            if (!auth.currentUser) return;
            db.collection('players').doc(auth.currentUser.uid).get().then(doc => {
                if (doc.exists) {
                    startGame({ ...gameLogic.getDefaultState(), ...doc.data() });
                } else {
                    const newState = gameLogic.getDefaultState();
                    startGame(newState);
                    gameLogic.save();
                }
            }).catch(e => { console.error("Error de carga:", e); startGame(gameLogic.getDefaultState()); });
        },
        updateLeaderboard: () => {
            if (!auth.currentUser) return;
            ...
            }).catch(e => console.error("Error de leaderboard:", e));
        },
        calculateProduction: matName => !state.unlockedMaterials.includes(matName) ? 0 : CONFIG.MATERIALS[matName].prodPerLevel * state.productionLevels[matName],
        getProductionUpgradeCost: matName => Math.ceil(CONFIG.MATERIALS[matName].upgradeCostBase * Math.pow(1.25, state.productionLevels[matName])),
        buyProductionUpgrade: matName => {
            const cost = gameLogic.getProductionUpgradeCost(matName);
            if (state.money >= cost) {
                state.money -= cost; state.productionLevels[matName]++;
        ...
        unlockMine: matName => {
            ...
            if (!state.unlockedMaterials.includes(matName) && state.money >= cost) {
                state.money -= cost; state.unlockedMaterials.push(matName);
                ui.showNotification(`Mina de ${CONFIG.MATERIALS[matName].name} desbloqueada`, 'success');
            }
        },
        unlockCity: cityId => {
            const city = CONFIG.CITIES[cityId], cost = city.unlockCost;
            if (!state.unlockedCities.includes(cityId) && state.money >= cost) {
                state.money -= cost; state.unlockedCities.push(cityId);
        ...
        sellAll: matName => {
            ...
            if (amount > 0) {
                const price = state.marketPrices[state.currentScreen][matName], earnings = price * amount;
                state.inventory[matName] -= amount; state.money += earnings;
                ui.showNotification(`Vendido ${amount.toLocaleString()} de ${matName} por $${earnings.toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 2 })}`, 'success');
            }
        },
        updateMarketPrices: (notify = true) => {
            for (const cityId in CONFIG.CITIES) {
                ...
                for (const matName in CONFIG.MATERIALS) {
                    const baseMod = CONFIG.CITIES[cityId].basePriceModifiers[matName], fluctuation = (Math.random() * 0.4) + 0.8;
                    state.marketPrices[cityId][matName] = CONFIG.MATERIALS[matName].baseValue * (baseMod * fluctuation);
                }
            }
            if (notify) { ui.showNotification("Precios de mercado actualizados", "success"); }
        },
        buyGood: goodName => {
            const good = CONFIG.TRADE_GOODS[goodName], city = CONFIG.CITIES[state.currentScreen], saleInfo = city.sells.find(i => i.good === goodName);
            if (!good || !saleInfo) return;
            const cost = good.baseValue * saleInfo.modifier;
            if (state.money >= cost) {
                state.money -= cost; state.inventory[goodName]++;
        ...
        completeContract: contractId => {
            ...
            if (Object.entries(contract.requirements).every(([mat, amount]) => state.inventory[mat] >= amount)) {
                Object.entries(contract.requirements).forEach(([mat, amount]) => state.inventory[mat] -= amount);
                state.money += contract.reward.money; state.completedContracts.push(contractId);
    ...
    Object.assign(ui, {
        showScreen: screenId => {
            ...
            if (CONFIG.CITIES[screenId]) {
                DOM.screens.city.classList.remove('hidden'); DOM.navButtons.map.classList.add('active');
            } else {
                DOM.screens[screenId].classList.remove('hidden'); DOM.navButtons[screenId].classList.add('active');
            }
            if (screenId === 'map') ui.renderLeaderboard();
        },
        render: () => {
            if (!state || typeof state.money === 'undefined') return;
            DOM.moneyDisplay.innerText = `$${Math.floor(state.money).toLocaleString()}`;
            DOM.locationDisplay.innerText = CONFIG.CITIES[state.currentScreen]?.name || (state.currentScreen === 'mine' ? 'La Mina' : 'Mapa');
            const mins = Math.floor(state.marketRefreshTimer / 60), secs = state.marketRefreshTimer % 60;
        ...
        renderMineScreen: () => {
            ...
            DOM.mineScreen.prodUpgrades.innerHTML = ''; DOM.mineScreen.mineUnlocks.innerHTML = '';
            for (const mName in CONFIG.MATERIALS) {
                if (state.unlockedMaterials.includes(mName)) {
                    const cost = gameLogic.getProductionUpgradeCost(mName), can = state.money >= cost;
                    ...
                    btn.innerText = `Mejorar ($${cost.toLocaleString()})`; btn.onclick = () => gameLogic.buyProductionUpgrade(mName);
                    el.appendChild(btn); DOM.mineScreen.prodUpgrades.appendChild(el);
                } else {
                    const cost = CONFIG.MATERIALS[mName].unlockCost, can = state.money >= cost;
                    const btn = document.createElement('button'); btn.className = `upgrade-btn ${can ? '' : 'disabled'}`;
                    btn.innerText = `Desbloquear Mina de ${mName} ($${cost.toLocaleString()})`; btn.onclick = () => gameLogic.unlockMine(mName);
        ...
        renderContractsBoard: () => {
            DOM.mineScreen.contracts.innerHTML = ''; CONFIG.CONTRACTS.forEach(c => {
                const isComp = state.completedContracts.includes(c.id); let reqsHtml = '', canComp = true;
                for (const mName in c.requirements) {
                    const reqAm = c.requirements[mName], hasAm = state.inventory[mName], hasEn = hasAm >= reqAm;
                    if (!hasEn) canComp = false;
                    reqsHtml += `<li class="${hasEn ? 'req-met' : 'req-not-met'}">${(CONFIG.MATERIALS[mName] || CONFIG.TRADE_GOODS[mName]).name}: ${Math.floor(hasAm).toLocaleString()} / ${reqAm.toLocaleString()}</li>`;
                }
                ...
                if (!isComp) {
                    ...
                    btn.innerText = 'Completar Contrato'; btn.onclick = () => gameLogic.completeContract(c.id);
                    el.appendChild(btn);
                }
        ...
        renderMapScreen: () => {
            ...
            for (const cId in CONFIG.CITIES) {
                const c = CONFIG.CITIES[cId], btn = document.createElement('button'); btn.className = 'city-btn';
                if (state.unlockedCities.includes(cId)) {
                    btn.innerText = `Viajar a ${c.name}`; btn.onclick = () => ui.showScreen(cId);
                } else {
                    const can = state.money >= c.unlockCost; btn.className += ` ${can ? '' : 'disabled'}`;
                    btn.innerText = `Desbloquear ruta a ${c.name} ($${c.unlockCost.toLocaleString()})`; btn.onclick = () => gameLogic.unlockCity(cId);
                }
        ...
        renderLeaderboard: () => {
            ...
            db.collection('leaderboard').orderBy('money', 'desc').limit(10).get().then(snap => {
                if (snap.empty) { DOM.mapScreen.leaderboard.innerHTML = 'Sé el primero.'; return; }
                DOM.mapScreen.leaderboard.innerHTML = ''; let rank = 1;
                snap.forEach(doc => {
                    const data = doc.data(), isPlayer = auth.currentUser && doc.id === auth.currentUser.uid;
                    ...
                    DOM.mapScreen.leaderboard.appendChild(el);
                });
            }).catch(e => { console.error(e); DOM.mapScreen.leaderboard.innerHTML = 'Error al cargar.'; });
        },
        renderCityScreen: () => {
            DOM.cityScreen.name.innerText = CONFIG.CITIES[state.currentScreen].name;
            ui.renderCityMarket(); ui.renderCityGoodsForSale();
        },
        renderCityMarket: () => {
            DOM.cityScreen.market.innerHTML = ''; for (const mName in CONFIG.MATERIALS) {
                if (state.inventory[mName] && Math.floor(state.inventory[mName]) > 0) {
                    const am = Math.floor(state.inventory[mName]), p = state.marketPrices[state.currentScreen][mName];
                    const el = document.createElement('div'); el.className = 'resource-line';
                    el.innerHTML = `<span>${mName} (${am.toLocaleString()}) - <b>$${p.toFixed(2)}</b>/u</span>`;
                    const btn = document.createElement('button'); btn.innerText = 'Vender Todo'; btn.onclick = () => gameLogic.sellAll(mName);
                    el.appendChild(btn); DOM.cityScreen.market.appendChild(el);
                }
            }
        },
        renderCityGoodsForSale: () => {
            const c = CONFIG.CITIES[state.currentScreen]; DOM.cityScreen.goods.innerHTML = '';
            if (!c.sells || c.sells.length === 0) { DOM.cityScreen.goods.innerHTML = '<p>No hay mercancías a la venta.</p>'; return; }
            c.sells.forEach(item => {
                const g = CONFIG.TRADE_GOODS[item.good], cost = g.baseValue * item.modifier, can = state.money >= cost;
                const el = document.createElement('div'); el.className = 'resource-line';
